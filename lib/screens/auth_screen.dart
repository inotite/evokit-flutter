
import 'package:evokit/screens/forgot_pwd_screen.dart';
import 'package:evokit/screens/tab_screen.dart';
import 'package:flutter/material.dart';
import 'package:evokit/screens/sign_up_screen.dart';
import 'package:evokit/models/user.dart';

class AuthScreen extends StatefulWidget {

  @override
  _AuthScreenState createState() => _AuthScreenState();
}

class _AuthScreenState extends State<AuthScreen> {
  TextEditingController email = TextEditingController();
  TextEditingController password = TextEditingController();

  Widget logoWidget() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        Container(
          child: Padding(
            padding: EdgeInsets.only(
              left: 10, right: 10
            ),
            child: Text(
              "E",
              style: TextStyle(
                color: Colors.black,
                fontSize: 33.0,
                fontWeight: FontWeight.bold,
                decoration: TextDecoration.none
              ),
            ),
          ),
          decoration: BoxDecoration(
            border: Border.all(
              width: 4,
              color: Colors.black
            )
          ),
        ),
        SizedBox(width: 8,),
        Container(
          child: Padding(
            padding: EdgeInsets.only(
              top: 13.0, left: 20.0, bottom: 13.0, right: 20.0
            ),
            child: Text(
              "Kit".toUpperCase(),
              style: TextStyle(
                color: Colors.white,
                fontSize: 18.0,
                fontWeight: FontWeight.bold,
                decoration: TextDecoration.none
              ),
            )
          ),
          decoration: BoxDecoration(
            color: Color(0xFF4636A6)
          ),
        ),
      ],
    );
  }

  Widget signInWidget() {
    return Column(
      children: <Widget>[
        Padding(
          padding: EdgeInsets.only(left: 10, right: 10, top: 10),
          child: Column(
            children: <Widget>[
              TextField(
                decoration: InputDecoration(
                  hintText: "E-mail",
                  border: OutlineInputBorder(),
                ),
                controller: email,
              ),
              TextField(
                decoration: InputDecoration(
                  hintText: "Password",
                  border: OutlineInputBorder(),
                ),
                controller: password,
                obscureText: true,
              ),
              ButtonTheme(
                minWidth: double.infinity,
                height: 40,
                child: FlatButton(
                  onPressed: () async {
                    final response = await fetchUser(email.text, password.text);

                    if (response["success"] == false) return;
                    
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (BuildContext context) {
                          return TabScreen();
                        }
                      )
                    );
                  },
                  child: Text(
                    "Sign In".toUpperCase(),
                    style: TextStyle(
                      color: Colors.white,
                      fontWeight: FontWeight.bold
                    ),
                  ),
                  color: Color(0xFF4636A6),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8.0))
                ),
              ),
              ButtonTheme(
                minWidth: double.infinity,
                height: 40,
                child: FlatButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (BuildContext context) {
                          return ForgotPwdScreen();
                        }
                      )
                    );
                  },
                  child: Text(
                    "I Forgot Password".toUpperCase(),
                    style: TextStyle(
                      color: Color(0xFF4636A6),
                      fontWeight: FontWeight.bold
                    ),
                  ),
                )
              ),
            ]
            .map((c) => Container(
              padding: EdgeInsets.only(bottom: 10),
              child: c,
            ))
            .toList(),
          ),
        ),
        ButtonTheme(
          minWidth: double.infinity,
          height: 50,
          child: FlatButton(
            onPressed: () {
              // Navigator.pop(context);
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (BuildContext context) {
                    return SignUpScreen();
                  }
                )
              );
            },
            child: Text(
              "Create account".toUpperCase(),
              style: TextStyle(
                color: Color(0xFF4636A6),
                fontWeight: FontWeight.bold
              ),
            ),
          )
        )
      ],
    );
  }

  Widget languageWidget() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        FlatButton(
          onPressed: () {

          },
          child: Text(
            "English",
            style: TextStyle(
              color: Colors.blueGrey
            ),
          ),
        ),
        Text(
          "|",
          style: TextStyle(
            color: Colors.blueGrey
          ),
        ),
        FlatButton(
          onPressed: () {

          },
          child: Text(
            "中文",
            style: TextStyle(
              color: Color(0xFF4636A6)
            ),
          ),
        )
        
      ],
    );
  }

  Widget termWidget() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: <Widget>[
        FlatButton(
          onPressed: () {

          },
          child: Text(
            "Terms of Service",
            style: TextStyle(
              color: Color(0xFF4636A6)
            ),
          ),
        ),
        FlatButton(
          onPressed: () {

          },
          child: Text(
            "Privacy Policy",
            style: TextStyle(
              color: Color(0xFF4636A6)
            ),
          ),
        )
        
      ],
    );
  }

  Widget copyrightWidget() {
    return Text(
      "Evovor 2019 All rights reserved",
      style: TextStyle(
        fontSize: 14,
        color: Colors.grey
      ),
    );
  }

  @override
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        print("Auth | Back button pressed");
        return false;
      },
      child: Scaffold(
        body: Center(
          child: Padding(
            padding: EdgeInsets.all(16),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: <Widget>[
                logoWidget(),
                signInWidget(),
              ]
              .map((c) => Container(
                padding: EdgeInsets.only(bottom: 15),
                child: c
              ))
              .toList(),
            ),
          )
        ),
        backgroundColor: Colors.white,
      ),
    );
  }
}
